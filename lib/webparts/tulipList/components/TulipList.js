...
var TulipList = /** @class */ (function (_super) {
    ...
    TulipList.prototype._deleteListItem = function (item) {
        ...
        return this.props.context.spHttpClient.get(this.props.context.pageContext.web.absoluteUrl + ("/_api/web/lists/getbytitle('" + this.props.listName + "')/items(" + item.ID + ")?$select=Id"), SPHttpClient.configurations.v1)
            .then(function (response) {
            return response.json();
        })
            ...
            return _this.props.context.spHttpClient.post(endpoint, SPHttpClient.configurations.v1, request);
